...
```mermaid
flowchart TD
    A[Пользователь в Telegram] --> B[Запрос<br/>на естественном языке]
    B --> C[Telegram Bot<br/>Интерфейсный слой]
    
    C --> D[AgenticRAG Handler Ядро системы]
    
    D --> E{Фаза 1: Анализ запроса LLM (GigaChat)}
    E --> F["JSON-план: Ключевые навыки, Город Опыт работы, Должность"]
    ...
    subgraph Scoring [Система весов]
        L["Навыки (40%) Соответствие технологиям"]
        M["Опыт (25%) Требуемый стаж"]
        N["Локация (20%) Совпадение города"]
        O["Должность (15%) Релевантность позиции"]
    end
    ...
    Q --> R["Итоговый ответ:<br/>• Анализ релевантности<br/>• Ранжированный список<br/>• Ссылки на hh.ru"]
    
    R --> S[Пользователь<br/>получает результат]
