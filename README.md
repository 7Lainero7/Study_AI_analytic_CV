...
Система построена на принципах **AgenticRAG** (Retrieval-Augmented Generation с агентским управлением). В отличие от классического RAG, где поиск и генерация разделены, здесь языковая модель выступает как активный агент, управляющий всем процессом.

#### Ключевые отличия от классического RAG:

| Аспект | Классический RAG | AgenticRAG (наша система) |
|--------|------------------|---------------------------|
| **Управление** | Статичный pipeline | Динамическое агентское управление |
| **Анализ запроса** | Простой parsing | Глубокий семантический анализ LLM |
| **Планирование поиска** | Фиксированные запросы | Адаптивное планирование на лету |
| **Обработка результатов** | Прямая выдача | Контекстуальный анализ и фильтрация |
| **Обратная связь** | Отсутствует | Итеративное уточнение критериев |

### Модели и технологии

#### 1. Языковая модель: GigaChat
- **Модель**: GigaChat:latest
- **Температура**: 0.3 (баланс креативности и стабильности)
- **Контекстное окно**: 8192 токена
- **Использование**: Анализ запросов, планирование поиска, оценка релевантности

#### 2. Модель эмбеддингов: Sentence Transformers
- **Модель**: all-MiniLM-L6-v2
- **Размерность**: 384
- **Скорость**: ~1000 документов/сек на CPU
- **Качество**: 68.9 на benchmark STS
- **Применение**: Векторизация резюме и поисковых запросов

#### 3. Векторная база данных: ChromaDB
- **Тип**: Persistent client
- **Метрика**: Косинусное расстояние
- **Индексация**: HNSW (Hierarchical Navigable Small World)
- **Емкость**: До 1 млн векторов
- **Фильтрация**: Поддержка сложных условий по метаданным

#### 4. Фреймворк бота: aiogram
- **Версия**: 3.x
- **Асинхронность**: Полная поддержка async/await
- **Мидлвари**: Гибкая система промежуточных обработчиков
- **Состояния**: Управление состоянием диалога

### Процесс обработки данных

#### Этап 1: Извлечение навыков
Система анализирует несколько источников информации в каждом резюме:

1. **Прямые навыки** (поле `skills`) - явно указанные технологии
2. **Структурированные навыки** (`skills_by_level`) - с указанием уровня владения
3. **Текст описания опыта** - извлечение технологий через паттерны
4. **Раздел "О себе"** - дополнительные упоминания технологий

**Пример извлечения:**

```
Исходный текст: "Разработка на React и TypeScript, работа с Redux"
Извлечено: ["React", "TypeScript", "Redux"]
```


#### Этап 2: Нормализация
Технологии приводятся к стандартному виду:
- Регистр: `react` → `React`
- Синонимы: `react.js`, `reactjs` → `React`
- Языковые варианты: `реакт` → `React`
- Версии: `python 3.9` → `Python`

#### Этап 3: Формирование документа
Создается обогащенный текст для векторизации:

```
Должность: Frontend-разработчик
Навыки: React, TypeScript, Redux, JavaScript
Город: Москва
Опыт: 5 лет
Описание: Разработка SPA приложений на React...
```
...
#### Алгоритм поиска:
1. **Семантический поиск**: Поиск по эмбеддингам запроса
2. **Гибридный поиск**: Комбинация с keyword search по навыкам
3. **Рекурсивное извлечение**: Многоуровневое разбиение сложных запросов

#### Фильтрация:
```python
# Пример фильтров ChromaDB
filters = {
    "$and": [
        {"location": {"$eq": "москва"}},
        {"total_experience_months": {"$gte": 36}},
        {"$or": [
            {"all_skills": {"$contains": "react"}},
            {"all_skills": {"$contains": "react.js"}}
        ]}
    ]
}
```
...
При недостаточном количестве результатов система применяет многоуровневую fallback-стратегию, последовательно ослабляя критерии поиска:

1. **Ослабление требований к опыту**: Убирает фильтр минимального стажа
2. **Расширение локации**: Убирает требования к конкретному городу
3. **Упрощение навыков**: Ослабляет требования к обязательным технологиям
4. **Общие запросы**: Использует более широкие формулировки для поиска

### Система оценки релевантности

Резюме оцениваются по взвешенной формуле, где каждый критерий имеет определенный вес в общей оценке:

```
Total_Score = (Skills_Score × 0.4) + (Experience_Score × 0.25) + (Location_Score × 0.2) + (Position_Score × 0.15)
```


#### Детализация компонентов оценки:

**Skills_Score (0-40 баллов)**
- Полное совпадение всех требуемых навыков: 40 баллов
- Частичное совпадение ключевых навыков: 20-35 баллов
- Совпадение по смежным технологиям: 10-20 баллов

**Experience_Score (0-25 баллов)**
- Опыт работы превышает требования: 25 баллов
- Опыт составляет 70-99% от требований: 15-20 баллов
- Опыт составляет 50-69% от требований: 10-15 баллов

**Location_Score (0-20 баллов)**
- Точное совпадение города: 20 баллов
- Удаленная работа при запросе удаленной работы: 15 баллов
- Готовность к переезду в нужный город: 10 баллов

**Position_Score (0-15 баллов)**
- Точное совпадение должности: 15 баллов
- Частичное совпадение названия позиции: 8-12 баллов
- Смежная специализация: 5-8 баллов

### Оптимизация производительности

#### 1. Кэширование
Система использует многоуровневое кэширование для повышения скорости ответа:
- Кэш эмбеддингов часто используемых поисковых запросов
- Кэш результатов поиска с временем жизни 1 час
- Кэш анализов GigaChat для типовых запросов пользователей

#### 2. Пакетная обработка данных
Обработка документов выполняется батчами для эффективного использования ресурсов:
...
Все ключевые операции в системе выполняются асинхронно для максимальной производительности и отзывчивости:

#### Асинхронная архитектура обработки

**Асинхронные вызовы GigaChat API**
- Неблокирующие запросы к языковой модели
- Параллельная обработка нескольких анализов
- Таймауты и повторные попытки при сбоях сети
- Пул соединений для эффективного использования ресурсов

**Асинхронная обработка Telegram-сообщений**
- Одновременное обслуживание нескольких пользователей
- Неблокирующие ответы на команды и запросы
- Фоновая обработка длительных операций поиска
- Кэширование промежуточных результатов

#### Реализация асинхронности

Система использует `asyncio` и `async/await` паттерны:

```python
async def process_user_query(user_query: str) -> str:
    # Асинхронный анализ запроса
    analysis = await analyze_with_gigachat(user_query)
    
    # Асинхронный поиск в векторной БД
    results = await search_in_chromadb(analysis)
    
    # Асинхронная оценка релевантности
    ranked = await evaluate_relevance(results, analysis)
    
    # Асинхронное формирование ответа
    response = await format_response(ranked)
